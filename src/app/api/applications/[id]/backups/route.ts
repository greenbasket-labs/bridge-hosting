...
function serializeBackup(backup:{id:string;applicationId:string;status:string;storageRef:string|null;sizeBytes:bigint|null;completedAt:Date|null;createdAt:Date}){return {...backup,sizeBytes:backup.sizeBytes===null?null:Number(backup.sizeBytes)};}
...
export async function GET(_:Request,{params}:{params:Promise<{id:string}>}){
  try{
    ...
    const backups=await db.backup.findMany({where:{applicationId:id,status:{not:'EXPIRED'}},orderBy:{createdAt:'desc'},take:50});
    return NextResponse.json({backups:backups.map(serializeBackup)});
  }catch(e){return NextResponse.json({error:e instanceof Error?e.message:'Failed to load backups'},{status:500});}
}
